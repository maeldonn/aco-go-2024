...
func PartOne(input []string) (int, error) {
	...
	for y, line := range input {
		for x, char := range line {
			...
			antennas[char] = append(antennas[char], image.Point{x, y})
		}
	}
	...
	for _, points := range antennas {
		...
		for _, p1 := range points {
			...
			for _, p2 := range points {
				...
				computeAntinodes := func(curr image.Point, prev image.Point) {
					for curr.Y >= 0 && curr.Y < len(input) && curr.X >= 0 && curr.X < len(input[curr.Y]) {
						location[curr] = struct{}{}
						curr, prev = image.Point{X: 2*curr.X - prev.X, Y: 2*curr.Y - prev.Y}, curr
					}
				}

				computeAntinodes(image.Point{X: 2*p2.X - p1.X, Y: 2*p2.Y - p1.Y}, p2)
				computeAntinodes(image.Point{X: 2*p1.X - p2.X, Y: 2*p1.Y - p2.Y}, p1)
			}
		}
	}
